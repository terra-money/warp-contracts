...
use cosmwasm_std::{
    ...
    QueryRequest, Reply, Response, SubMsgResult, Uint128, Uint64, WasmMsg,
};
...
pub fn execute_job(deps: DepsMut, env: Env, msg: Reply) -> Result<Response, ContractError> {
    ...
    if finished_job.recurring {
            ...
            || finished_job.status == JobStatus::Failed)
        {
            ...
            let new_job =
                PENDING_JOBS().update(deps.storage, state.current_job_id.u64(), |s| match s {
                    None => Ok(Job {
                        ...
                        job_account: finished_job.job_account,
                    }),
