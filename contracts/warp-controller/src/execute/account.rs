use crate::contract::{REPLY_ID_FEE_ASSET_ACCOUNT_CREATION, REPLY_ID_EXECUTION_ASSET_ACCOUNT_CREATION};
use crate::state::{FEE_ASSET_ACCOUNTS, EXECUTION_ASSET_ACCOUNTS, CONFIG};
...
pub fn create_fee_asset_account(
    deps: DepsMut,
    env: Env,
    info: MessageInfo,
    ...
    if FEE_ASSET_ACCOUNTS().has(deps.storage, info.sender.clone()) {
        let account = FEE_ASSET_ACCOUNTS().load(deps.storage, info.sender.clone())?;
...
pub fn create_execution_asset_account(
    deps: DepsMut,
    env: Env,
    info: MessageInfo,
    data: CreateAccountMsg,
    ...
    let item: Result<Option<(Vec<u8>, controller::account::Account)>, cosmwasm_std::StdError> = EXECUTION_ASSET_ACCOUNTS()
        .idx
        .account
        .item(deps.storage, info.sender.clone());
    ...
    if EXECUTION_ASSET_ACCOUNTS().has(deps.storage, info.sender.clone()) {
        let account = EXECUTION_ASSET_ACCOUNTS().load(deps.storage, info.sender.clone())?;
    ...
    let submsg = SubMsg {
        id: REPLY_ID_EXECUTION_ASSET_ACCOUNT_CREATION,
        msg: CosmosMsg::Wasm(WasmMsg::Instantiate {
            admin: Some(env.contract.address.to_string()),
            code_id: config.warp_account_code_id.u64(),
            msg: to_binary(&account::InstantiateMsg {
