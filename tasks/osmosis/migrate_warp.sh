...
store_contract() {
    ...
    STORE_OUTPUT=$(yes $wallet_password | osmosisd tx wasm store "$contract_file" \
    ...
    QUERY_OUTPUT=$(osmosisd query tx --type=hash "$TXHASH" --node="$node" --chain-id="$chain_id" --output json)
...
migrate_contract() {
    ...
    MIGRATE_OUTPUT=$(yes "$wallet_password" | osmosisd tx wasm migrate "$contract_address" "$new_code_id" "$migrate_msg" \
        --from="$signer_key_acc_address" \
        --chain-id="$chain_id" \
        --node="$node" \
        --gas-prices="$gas_price" --gas=10000000 \
