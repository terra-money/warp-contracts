...
store_contract() {
    ...
    STORE_OUTPUT=$(yes $wallet_password | oraid tx wasm store "$contract_file" \
    ...
    QUERY_OUTPUT=$(oraid query tx --type=hash "$TXHASH" --node="$node" --chain-id="$chain_id" --output json)
...
instantiate_contract() {
    ...
    INSTANTIATE_OUTPUT=$(yes "$wallet_password" | oraid tx wasm instantiate $code_id "$instantiate_msg" \
...
fetch_account_tracker_address() {
    ...
    QUERY_OUTPUT=$(oraid query wasm contract-state smart $controller_contract '{"query_config":{}}' --node="$node" --chain-id="$chain_id" --output json)
